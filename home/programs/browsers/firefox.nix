{ pkgs, config, lib, isLinux ? true, isDarwin ? false, ... }:
{
  programs.firefox = {
    ...
    profiles.${config.home.username} = {
      ...
      # Bluloco theme (Dark & Light) via userChrome.css with automatic theme detection
      userChrome = ''
        /* Bluloco Theme for Firefox - Automatic theme switching via prefers-color-scheme */
